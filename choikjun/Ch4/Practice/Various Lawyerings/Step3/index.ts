...
export type FromReason = {
	from: "defendant" | "plaintiff";
	reason: string;
};

export type PostTrial = FromReason & {
	step: "post-trial";
	classification: "acquittal" | "correction" | "new trial";
};

export type PreTrial = FromReason & {
	step: "pre-trial";
	classification: "suppress" | "dismiss" | "venue";
};

export type Trial = PreTrial | PostTrial;

export type Allowed = Trial & {
	deliberationHours: number;
	status: "allowed";
};

export type Denied = Trial & {
	deliberationHours: number;
	status: "denied";
	annoyedJustice: boolean;
};

export type Pending = Trial & {
	status: "pending";
	estimatedDeliberationHours: number;
};

export type Motion = Allowed | Denied | Pending;
