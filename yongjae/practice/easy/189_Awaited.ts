...
// 동영 멘토님: 콜백 함수가 일반 함수냐 비동기 함수냐에 따라서 타이핑 해줘야 할 때 활용한다
// T가 Promise 타입이 가져야 하는 내부 속성을 모두 가지지 않을 수도 있기 때문에 PromiseLike로 넓은 의미의 타입 적용을 해야 한다.

type MyAwaited<T extends PromiseLike<any>> = T extends PromiseLike<infer K> ? K extends PromiseLike<any> ? MyAwaited<K> : K: never; 
