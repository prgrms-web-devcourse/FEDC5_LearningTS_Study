...
type StreamInfo = Info & {
  type: "stream";
};
type StreamDown = StreamInfo & {
  area: "begin";
  downstream: Types;
};
type StreamUp = StreamInfo & {
  area: "end";
  upstream: Types;
};
type StreamMiddle = StreamInfo & {
  area: "middle";
  upstream: Types;
  downstream: Types;
};
type Streams = StreamDown | StreamUp | StreamMiddle;
type Types = Town | Clearing | Streams | Path;

let current: Types | undefined = {
  name: "Woesong Bridge",
  proximity: 100,
  through: {
    area: "middle",
