...
사용자 속성을 추가로 갖는 함수를 설명하는데 사용할 수도 있다
...
function keepsTrackOfCalls() {
  keepsTracOfCalls.count += 1;
  console.log("!?");
}

keepsTrackOfCalls.count = 0; // number타입인 counter 속성이 주어져서

hasCallCount = keepsTrackOfCalls; // 이렇게 FuncWithCount인터페이스에 할당 가능함
```

> ++ 스터디 중 추가 질의응답
>
> Q. keepsTrackOfCalls() 내부에서 return 1 등 숫자를 return 하는데 FuncWithCount인터페이스에서 오류로 판단하지 않는다. number가 아닌데 왜?? + string 으로 선언하면 오류가 난다. 왜?
>
> A. void는 "void를 반환하도록 선언된 타입 위치에 전달된 함수가 반환된 모든 값을 무시하도록 설정할 때 사용"된다!!
